...
JsonEscape(char *in, int *outlen)
{
  ...
  _SFCB_ENTER(TRACE_CIMRS, "JsonEscape");
...
value2json(CMPIData d, UtilStringBuffer * sb, int wv)
{
  ...
  if (d.type & CMPI_ARRAY) {
    ...
    if ((d.type & (CMPI_UINT | CMPI_SINT)) == CMPI_UINT) {
      ...
      sp = JsonEscape(d.value.chars, &splen);
      if (sp)
        freesp = 1;
    } else if (d.type == CMPI_string) {
      sp = JsonEscape((char *) d.value.string->hdl, &splen);
      if (sp)
        freesp = 1;
      ...
      instance2json(d.value.inst, sb, 0);
      SFCB_APPENDCHARS_BLOCK(sb, "]]>");
      splen = 0;
    } else {
      mlogf(M_ERROR, M_SHOW, "%s(%d): invalid value2json %d-%x\n", __FILE__,
...
static int      keyBinding2json(CMPIObjectPath * op, UtilStringBuffer * sb);

static int
lnsPath2json(CMPIObjectPath * ci, UtilStringBuffer * sb)
{
  _SFCB_ENTER(TRACE_CIMRS, "lnsPath2json");
...
static int
nsPath2json(CMPIObjectPath * ci, UtilStringBuffer * sb)
{
  _SFCB_ENTER(TRACE_CIMRS, "nsPath2json");
...
int
instanceName2json(CMPIObjectPath * cop, UtilStringBuffer * sb)
{
  _SFCB_ENTER(TRACE_CIMRS, "instanceName2json");
  SFCB_APPENDCHARS_BLOCK(sb, "<INSTANCENAME CLASSNAME=\"");
  sb->ft->appendChars(sb, opGetClassNameChars(cop));
  SFCB_APPENDCHARS_BLOCK(sb, "\">\n");
  keyBinding2json(cop, sb);
...
static int
refValue2json(CMPIObjectPath * ci, UtilStringBuffer * sb)
{
  _SFCB_ENTER(TRACE_CIMRS, "refValue2json");
...
static void
method2json(CMPIType type, CMPIString *name, char *bTag, char *eTag,
           UtilStringBuffer * sb, UtilStringBuffer * qsb)
{
  _SFCB_ENTER(TRACE_CIMRS, "method2json");
  ...
  if (data->type & CMPI_ARRAY) {
    ...
    if (*type == '*') {
      ...
      if (inst && data->value.ref) {
        refValue2json(data->value.ref, sb);
      }
    }
      ...
      if (data->value.inst) {
        SFCB_APPENDCHARS_BLOCK(sb, "<VALUE>");
        SFCB_APPENDCHARS_BLOCK(sb, "<![CDATA[");
        instance2json(data->value.inst, sb, 0);
        SFCB_APPENDCHARS_BLOCK(sb, "]]>");
        SFCB_APPENDCHARS_BLOCK(sb, "</VALUE>\n");
      }
    }
...
static void
quals2json(unsigned long quals, UtilStringBuffer * sb)
{
...
static void
param2json(CMPIParameter * pdata, CMPIConstClass * cls, ClParameter * parm,
  ...
  if (flags & FL_includeQualifiers) {
    ...
    for (i = 0; i < m; i++) {
      ClClassGetMethParamQualifierAt(cl, parm, i, &data,
                                     (char **) &qname.hdl);
      DATA2JSON(&data, cls, &qname, NULL, "<QUALIFIER NAME=\"",
               "</QUALIFIER>\n", qsb, NULL, 0, 0);
    }
  }
...
int
cls2json(CMPIConstClass * cls, UtilStringBuffer * sb, unsigned int flags)
{
  ClClass        *cl = (ClClass *) cls->hdl;
  int             i,
                  m,
  ...
  _SFCB_ENTER(TRACE_CIMRS, "cls2json");
  ...
  for (i = 0, m = ClClassGetPropertyCount(cl); i < m; i++) {
    ...
    if (data.type & CMPI_ARRAY)
      ...
      type = dataType(data.type);
      if (*type == '*') {
        DATA2JSON(&data, cls, name, refName, "<PROPERTY.REFERENCE NAME=\"",
                 "</PROPERTY.REFERENCE>\n", sb, qsb, 0, 0);
      } else
        DATA2JSON(&data, cls, name, NULL, "<PROPERTY NAME=\"",
                 "</PROPERTY>\n", sb, qsb, 0, 0);
    }
    CMRelease(name);
  }
  ...
  for (i = 0, m = ClClassGetMethodCount(cl); i < m; i++) {
    ...
    if (flags & FL_includeQualifiers) {
      for (q = 0, qm = ClClassGetMethQualifierCount(cl, i); q < qm; q++) {
        ...
        DATA2JSON(&qdata, cls, name, NULL, "<QUALIFIER NAME=\"",
                 "</QUALIFIER>\n", qsb, NULL, 0, 0);
      }
    }
    ...
    for (p = 0, pm = ClClassGetMethParameterCount(cl, i); p < pm; p++) {
      ...
      param2json(&pdata, cls, parm, name, qsb, flags);
    }

    method2json(mtype, mname, "<METHOD NAME=\"", "</METHOD>\n", sb, qsb);
  }
...
int
instance2json(CMPIInstance *ci, UtilStringBuffer * sb, unsigned int flags)
{
  ...
  _SFCB_ENTER(TRACE_CIMRS, "instance2json");
  ...
  for (i = 0; i < m; i++) {
    ...
    if (data.type & CMPI_ARRAY) {
      DATA2JSON(&data, ci, &name, NULL, "<PROPERTY.ARRAY NAME=\"",
               "</PROPERTY.ARRAY>\n", sb, qsb, 1, 0);
    } else {
      type = dataType(data.type);
      if (*type == '*')
        DATA2JSON(&data, ci, &name, NULL, "<PROPERTY.REFERENCE NAME=\"",
                 "</PROPERTY.REFERENCE>\n", sb, qsb, 1, 0);
      else
        DATA2JSON(&data, ci, &name, NULL, "<PROPERTY NAME=\"",
                 "</PROPERTY>\n", sb, qsb, 1, 0);
    }
...
int
args2json(CMPIArgs * args, UtilStringBuffer * sb)
{
  int             i,
                  m;

  _SFCB_ENTER(TRACE_CIMRS, "args2json");
  ...
  for (i = 0; i < m; i++) {
    ...
    DATA2JSON(&data, args, name, NULL, "<PARAMVALUE NAME=\"",
             "</PARAMVALUE>\n", sb, NULL, 1, 1);
...
static int
keyBinding2json(CMPIObjectPath * op, UtilStringBuffer * sb)
{
  int             i,
                  m;
  _SFCB_ENTER(TRACE_CIMRS, "keyBinding2json");
...
static int
className2json(CMPIObjectPath * op, UtilStringBuffer * sb)
{
...
int
enum2json(CMPIEnumeration *enm, UtilStringBuffer * sb, CMPIType type,
...
#ifdef HAVE_QUALREP
int
qualifierDeclaration2json(CMPIQualifierDecl * q, UtilStringBuffer * sb)
{
  ClQualifierDeclaration *qual = (ClQualifierDeclaration *) q->hdl;
  CMPIData        data;

  _SFCB_ENTER(TRACE_CIMRS, "qualifierDeclaration2json");
  ...
  if (data.state == CMPI_goodValue) {
    if (data.type & CMPI_ARRAY) {
      ...
      for (i = 0;
        ...
        value2json(data.value.array->ft->
...
int
qualiEnum2json(CMPIEnumeration *enm, UtilStringBuffer * sb)
{
  CMPIQualifierDecl *q;

  _SFCB_ENTER(TRACE_CIMRS, "qualiEnum2json");
  while (CMHasNext(enm, NULL)) {
    q = CMGetNext(enm, NULL).value.dataPtr.ptr;
    qualifierDeclaration2json(q, sb);
  }
  _SFCB_RETURN(0);
}
#endif
