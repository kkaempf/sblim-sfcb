...
processThreadFunc(void *params)
{
  ...
  if (getenv("SFCB_PAUSE_HTTP"))
    for (p->breakloop = 0; p->breakloop == 0;) {
  ...
  do {
    ...
    p->isReady =
	select(p->conn_fd.socket + 1, &p->httpfds, NULL, NULL,
	       &p->httpTimeout);
    if (p->isReady == 0) {
      _SFCB_TRACE(1,
		  ("--- HTTP connection timeout, quit %d ", currentProc));
      break;
    } else if (p->isReady < 0) {
...
handleHttpRequest(int connFd)
{
  ...
  if (doFork) {
    ...
    sessionId++;
    // r = fork();
    struct processThreadParams ptparams =
	{ breakloop, conn_fd, connFd, httpfds, httpTimeout, isReady };
  ...
  // moved this hunk to processThreadFunc for process thread to work on;
  // this block remains for when doFork=0
  // if (r == 0) {
  if (doFork == 0) {
	...
	if (sslerr == SSL_ERROR_WANT_WRITE ||
	  ...
	  if (sslerr == SSL_ERROR_WANT_WRITE) {
	    isReady =
		select(connFd + 1, NULL, &httpfds, NULL, &httpTimeout);
	  } else {
	    isReady =
		select(connFd + 1, &httpfds, NULL, NULL, &httpTimeout);
	  }
	  if (isReady == 0) {
...
#else				/* end threads section, start forked
				 * process section */
...
int
httpDaemon(int argc, char *argv[], int sslMode, int sfcbPid)
{
