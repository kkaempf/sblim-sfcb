...
static const char *metaDB =
    "version=0.1;name=sfcb;maxcon=1000;mathfunc=keine,nochnich;sqlkeywords=selbe,wie,in,db2;sqlstate=nochnichtstandartisiert;stringfunc=comma,list;systemfunc=noch,nichts;datetimefunc=noch,nichts$$\n";
...
handleDbpSession(int connFd)
{
  ...
  // printf("dbpProcSem %p dbpProcId: %d hMax: %d doFork:
...
{                               // int argc, char *argv[], int sslMode) {
  ...
  sfcbSSLMode = sslMode;
  if (sslMode)
    processName = "DBPS-Daemon";
  else
    processName = "DBP-Daemon";
   ...
   * procs, port; int listenFd, connFd;
   * 
   * 
   * name = "sfcBroker"; //debug = 1; //doFork = 1;
   * 
   * _SFCB_ENTER(TRACE_DBPDAEMON, "dbpDaemon");
   * 
   * //setupControl(NULL); processName="DBP Daemon"; //Variable aus
   * sfcBroker, dieser neue Prozess hat noch keinen zugewiesen bekommen
   * 
   * //if (getControlNum("httpPort", &port)) port = 5980; //hBase=htBase;
   * //hMax=htMax;
   * 
   * //initndpProcCtl(procs);
   * 
   * 
   * cp = strrchr(name, '/'); if (cp != NULL) ++cp; else cp = name; name = 
   * cp;
   * 
   * 
   * printf("--- %s DBP Daemon V" sfcdbpDaemonVersion " started - %d -
   * port %ld\n", name, getpid(),port);
   * 
   * 
   * 
   * listenFd = socket(PF_INET, SOCK_STREAM, IPPROTO_TCP); sin_len =
   * sizeof(sin);
   * 
   * ru = 1; setsockopt(listenFd, SOL_SOCKET, SO_REUSEADDR, (char *) &ru,
   * sizeof(ru));
   * 
   * memset(&sin,0,sin_len);
   * 
   ...
   * 
   * if (bind(listenFd, (struct sockaddr *) &sin, sin_len) ||
   * listen(listenFd, 0)) {//Test, ob Port frei ist printf("cannot listen
   * on port %ld", port); exit(3); }
   * 
   * //if (!debug) { int rc = fork(); if (rc == -1) { perror("fork
   * daemon"); exit(1); } else if (rc) exit(0); //} // memInit();
   * //printf("jetzt kann sich jemand verbinden\n"); // setSignal(SIGCHLD, 
   * handleSigChld,0);
   * 
   * for (;;) {
   * 
   * listen(listenFd, 1); sz = sizeof(sin); if ((connFd = accept(listenFd, 
   * (__SOCKADDR_ARG) & sin, &sz))<0) { if (errno == EINTR || errno ==
   * EAGAIN) continue; perror("accept error"); // _SFCB_ABORT(); }
   * _SFCB_TRACE(1, ("--- Processing dbp request"));
   * 
