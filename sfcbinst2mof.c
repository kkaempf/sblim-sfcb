...
help(char *name)
{
  ...
  printf
      ("  -o <output file>   full path to output file. Path must exist, with write rights.  (default = stdout)\n");
  printf
  ...
  printf
      ("  -r <repository dir> use alternate repository (override sfcb cfg file). (default = %s/registration/repository)\n",
       SFCB_STATEDIR);
}
