...
int
queryInput(char *buffer, int *done, int requested)
{
...
void
sfcQueryErr(char *s1, char *s2, char *s3)
{
...
static char   **
ensureCharsListSpace(QLStatement * qs, char ***in, int *max, int nxt)
{
  char          **list = *in;
  if (*max - 1 <= nxt) {
    *max = *max * 2;
...
static void
qsSetAllProperties(QLStatement * st, int all)
{
  st->allProps = all;
}
...
static char   **
qsGetFromClassList(QLStatement * st)
{
  return st->fClasses;
}

static void
qsSetWhereCondition(QLStatement * st, QLOperation * op)
{
  st->where = op;
...
static int
qsTestPropertyClass(QLStatement * st, char *cl)
{
  int             i;
...
void           *
qsAlloc(QLStatement * qs, unsigned int size)
{
