...
// die erste Spalte, die nicht vereingbar ist. die mod 2000 999 Stellen
// codieren die Spaltennr 
#define CDEFERR 3000
...
Column         *newColumn(const char *tableName, const char *tableAlias, const char *colName,
                          const char *colAlias, int colSQLType, int isKey);
...
UpdIns         *newUpdIns(const char *tname, UtilList * colList,
                          UtilList * assignmentList, UtilList * where);
Insert         *newInsert(const char *tname);
ClassDef       *newClassDef(int fieldCount, const char *className,
...
int             createClass(const char *ns, const char *cn, const char *scn,
                            UtilList * colDef);
...
int             updateInstance(const char *ns, const char *cn, UtilList * al,
                               UtilList * kl);
UtilList       *getClassNames(const char *ns, const char *filter);
UtilList       *invokeMethod(const char *ns, const char *cn, const char *mn, UtilList * pl,
                             UtilList * kl);
ClassDef       *getClassDef(const const char *ns, const const char *cn);

/*
 * Der Teil des SQL-Prozessors, der die Metaanfragen bearbeitet
 */
/*
 * geht über den Parser, das Ergebnis-Resultset muss noch als Antwort
 * codiert werden
 */
ResultSet      *createRS(const char *query, int *rc);
/*
 * die Anfrage wird innerhalb der Funktion bearbeitet und gleich als
 * Antworstring codiert
 */
char           *processMetaTables(const char *filter, const char *db);
char           *processMetaColumns(const char *filter, const char *filtercol,
                                   const char *db);
