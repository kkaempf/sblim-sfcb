...
static int
getChars(XmlBuffer * xb, const char *s)
{
  int             l = strlen(s);
...
static int
getChar(XmlBuffer * xb, const char c)
{
  if (*xb->cur++ == c)
    return *(xb->cur - 1);
  xb->cur--;
...
/*
 * skip nulled char and whitespace, see if we're on a new tag and return
 * a pointer, or NULL otherwise 
 */
static char    *
nextTag(XmlBuffer * xb)
{
  if (xb->nulledChar) {
    xb->nulledChar = 0;
    return xb->cur + 1;
...
/*
 * compares two strings return 1: n begins with t and following char is
 * alphanumeric return 0: otherwise 
 */
static int
nextEquals(const char *n, const char *t)
{
  int             l = strlen(t);
  if (strncmp(n, t, l) == 0) {
    if (!isalnum(*(n + l))) {
...
/*
 * static int xmlHdr(XmlBuffer *xb) { XmlElement
 * elm[3]={{"version"},{"encoding"},{NULL}}; XmlAttr vals[2];
 * xb->eTagFound = 0;
 * 
 * skipWS(xb); if (getChars(xb,"<?xml")) { if
 * (attrsOk(xb,elm,vals,"?xml",ZTOK_CIM)) return 1; } return 0; }
 * 
 * static int skipElement(XmlBuffer *xb, const char *t) { char
 * eTag[256]="</"; strcat(eTag,t); strcat(eTag,">"); char
 * *e=strstr(xb->cur,eTag); if (e) { xb->cur=e+strlen(eTag); return 1; }
 * return 0; }
 * 
 * static int testAttr(XmlBuffer *xb, char *w, char *val, int xCase) {
 * char *end=strchr(xb->cur,'>'),*tcur=xb->cur,dlm,*a=NULL; int
 * l=strlen(val),rv=0;
 * 
 * *end=0; a=strstr(xb->cur,w); if (a) { xb->cur=a+strlen(w); skipWS(xb);
 * dlm=*(xb->cur); if ((xCase && strncmp(xb->cur+1,val,l)==0) || (xCase==0 
 * && strncasecmp(xb->cur+1,val,l)==0)) { if (*(xb->cur+l+1)==dlm) rv=1; }
 * } *end='>'; xb->cur=tcur; return rv; }
 * 
 * static int endTag(XmlBuffer *xb, const char *t, const XmlAttr *a) { char 
 * *start=xb->cur; if (a && xb->eTagFound) return 1; skipWS(xb); if
 * (getChars(xb,"</")) { skipWS(xb); if (getWord(xb,t,1)) { skipWS(xb); if
 * (getChar(xb,'>')) return 1; } } xb->cur=start; return 0; }
 * 
 */

static int
getWord(XmlBuffer * xb, const char *w, int xCase)
{
  int             l = strlen(w);
...
static int
tagEquals(XmlBuffer * xb, const char *t)
{
  char           *start = NULL;
  int             sz = 0;
  if (*xb->cur == 0) {
...
#define WORDLEN 20
   ...
   */
  int             wlen =
      (xb->cur < (xb->last - WORDLEN)) ? WORDLEN : (xb->last - xb->cur);
...
xmlUnescape(char *buf, char *end)
{
  ...
  if (buf[1] == '#') {
    ...
    *buf = unescchar;
    // now move rest of string, after the escaped character we just
    // converted, to follow
    // newly placed unescaped character
    memmove(buf + 1, buf + diflen, (int) (end - buf - diflen + 1));
    // and return the number of characters we removed from the total
    // string len
    return diflen - 1;
  } else {
    unsigned int i = 0;
...
getContent(XmlBuffer * xb)
{
  ...
  /*
   * store the char we found, set it to null to use as a marker in the
   * code below 
   */
  ...
  if (trimws) {
    ...
    */
    ...
    if (start == NULL)
      ...
      // instead? start==NULL shouldn't happen, ever.
    /*
...
static int
procMessage(YYSTYPE * lvalp, ParserControl * parm)
{
  static XmlElement elm[] = {
    {"ID"},
    {"PROTOCOLVERSION"},
...
static int
unsupported(ParserControl * parm)
{
  longjmp(parm->env, 5);
  return 0;
}
...
static int
procMethodCall(YYSTYPE * lvalp, ParserControl * parm)
{
  static XmlElement elm[] = {
    {"NAME"},
    {NULL}
...
static int
procNameSpace(YYSTYPE * lvalp, ParserControl * parm)
{
  static XmlElement elm[] = {
    {"NAME"},
    {NULL}
...
static int
procParamValue(YYSTYPE * lvalp, ParserControl * parm)
{
  static XmlElement elm[] = {
    {"NAME"},
    {"PARAMTYPE"},
...
static int
procClassName(YYSTYPE * lvalp, ParserControl * parm)
{
  static XmlElement elm[] = {
    {"NAME"},
    {NULL}
...
static int
procInstanceName(YYSTYPE * lvalp, ParserControl * parm)
{
  static XmlElement elm[] = {
    {"CLASSNAME"},
    {NULL}
...
static int
procKeyBinding(YYSTYPE * lvalp, ParserControl * parm)
{
  static XmlElement elm[] = {
    {"NAME"},
    {NULL}
...
static int
procInstance(YYSTYPE * lvalp, ParserControl * parm)
{
  static XmlElement elm[] = {
    {"CLASSNAME"},
    {NULL}
...
static int
procClass(YYSTYPE * lvalp, ParserControl * parm)
{
  static XmlElement elm[] = {
    {"NAME"},
    {"SUPERCLASS"},
...
static int
procKeyValue(YYSTYPE * lvalp, ParserControl * parm)
{
  static XmlElement elm[] = {
    {"VALUETYPE"},
    {"TYPE"},
...
static int
procHost(YYSTYPE * lvalp, ParserControl * parm)
{
  static XmlElement elm[] = {
    {NULL}
  };
...
static int
procValue(YYSTYPE * lvalp, ParserControl * parm)
{
  static XmlElement elm[] = {
    {NULL}
  };
...
procValueArray(YYSTYPE __attribute__ ((unused)) *lvalp, ParserControl * parm)
{
  ...
  if (tagEquals(parm->xmb, "VALUE.ARRAY")) {
    if (attrsOk(parm->xmb, elm, attr, "VALUE.ARRAY", ZTOK_VALUEARRAY)) {
...
static int
procValueNamedInstance(YYSTYPE * lvalp, ParserControl * parm)
{
  static XmlElement elm[] = {
    {NULL}
  };
...
static int
procInstancePath(YYSTYPE * lvalp, ParserControl * parm)
{
  static XmlElement elm[] = {
    {NULL}
  };
...
static int
procValueReference(YYSTYPE * lvalp, ParserControl * parm)
{
  static XmlElement elm[] = {
    {NULL}
  };
...
static int
procQualifierDeclaration(YYSTYPE * lvalp, ParserControl * parm)
{
  static XmlElement elm[] = {
    {"NAME"},
    {"TYPE"},
...
static int
procQualifier(YYSTYPE * lvalp, ParserControl * parm)
{
...
static int
procScope(YYSTYPE * lvalp, ParserControl * parm)
{
  static XmlElement elm[] = {
    {"CLASS"},
    {"ASSOCIATION"},
...
static int
procProperty(YYSTYPE * lvalp, ParserControl * parm)
{
  static XmlElement elm[] = { {"NAME"},
  {"TYPE"},
  {"CLASSORIGIN"},
...
static int
procPropertyArray(YYSTYPE * lvalp, ParserControl * parm)
{
...
static int
procPropertyReference(YYSTYPE * lvalp, ParserControl * parm)
{
  static XmlElement elm[] = { {"NAME"},
  {"REFERENCECLASS"},
  {"CLASSORIGIN"},
...
static int
procMethod(YYSTYPE * lvalp, ParserControl * parm)
{
  static XmlElement elm[] = { {"NAME"},
  {"TYPE"},
  {"CLASSORIGIN"},
...
static int
procParam(YYSTYPE * lvalp, ParserControl * parm)
{
  static XmlElement elm[] = { {"NAME"},
  {"TYPE"},
  {NULL}
...
static int
procParamArray(YYSTYPE * lvalp, ParserControl * parm)
{
  static XmlElement elm[] = { {"NAME"},
  {"TYPE"},
  {"ARRAYSIZE"},
...
static int
procParamRef(YYSTYPE * lvalp, ParserControl * parm)
{
  static XmlElement elm[] = { {"NAME"},
  {"REFERENCECLASS"},
  {NULL}
...
static int
procParamRefArray(YYSTYPE * lvalp, ParserControl * parm)
{
  static XmlElement elm[] = { {"NAME"},
  {"REFERENCECLASS"},
  {"ARRAYSIZE"},
...
static Tags     tags[] = {
  ...
  {"QUALIFIER.DECLARATION", procQualifierDeclaration,
   ZTOK_QUALIFIERDECLARATION},
...
int
yylex(YYSTYPE * lvalp, ParserControl * parm)
{
  int             i,
                  m,
                  rc;
  ...
  for (;;) {
    ...
    /*
     * found a tag, reset eTagFound flag, return the token for the tag
     * that was found 
     */
    ...
    /*
     * found an ending tag, loop from 0-#ofTags, return the token for the
...
static void
freeNameSpacePath(XtokNameSpacePath * op)
{
  if (op->nameSpacePath) {
...
static void
freeKeyBinding(XtokKeyBinding * op)
{
...
static void
freeKeyBindings(XtokKeyBindings * op)
{
  int             i;
  if (op->keyBindings) {
    for (i = 0; i < op->next; i++) {
...
static void
freeInstanceName(XtokInstanceName * op)
{
  freeKeyBindings(&op->bindings);
}

static void
freeLocalInstancePath(XtokLocalInstancePath * op)
{
  if (op->path) {
    free(op->path);
  }
...
static void
freeInstancePath(XtokInstancePath * op)
{
...
static void
freeReference(XtokValueReference * op)
{
  switch (op->type) {
...
static void
freeRefArray(XtokValueRefArray * op)
{
  int             i;
  if (op->values) {
    for (i = 0; i < op->next; i++) {
...
freeValue(XtokValue * op)
{
   ...
   * freeClass(op->class); free(op->class); }
   */
}

static void
freeArray(XtokValueArray * op)
{
  int             i;
  if (op->values) {
    for (i = 0; i < op->next; i++) {
...
static void
freeParamValue(XtokParamValue * op)
{
...
static void
freeParamValues(XtokParamValues * op)
{
...
static void
freeQualifier(XtokQualifier * op)
{
...
static void
freeQualifiers(XtokQualifiers * op)
{
  XtokQualifier  *qv = op->first,
      *dv;
  while (qv) {
    freeQualifier(qv);
    dv = qv;
...
static void
freeProperties(XtokProperties * op)
{
  XtokProperty   *pv = op->first,
      *dv;
  while (pv) {
    freeProperty(pv);
    dv = pv;
...
static void
freeParam(XtokParam * op)
{
  freeQualifiers(&op->qualifiers);
}

static void
freeParams(XtokParams * op)
{
  XtokParam      *pv = op->first,
      *dv;
  while (pv) {
    freeParam(pv);
    dv = pv;
...
static void
freeMethods(XtokMethods * op)
{
  XtokMethod     *mv = op->first,
      *dv;
  while (mv) {
    freeMethod(mv);
    dv = mv;
...
static void
freeMethodCall(XtokMethodCall * op)
{
  if (op->instName) {
    freeInstanceName(&op->instanceName);
  }
...
static void
freeGetClass(XtokGetClass * op)
{
  freeArray(&op->propertyList);
}

static void
freeCreateClass(XtokCreateClass * op)
{
...
static void
freeGetInstance(XtokGetInstance * op)
{
  freeInstanceName(&op->instanceName);
  freeArray(&op->propertyList);
}
...
static void
freeCreateInstance(XtokCreateInstance * op)
{
  freeInstance(&op->instance);
}

static void
freeModifyInstance(XtokModifyInstance * op)
{
...
static void
freeDeleteInstance(XtokDeleteInstance * op)
{
  freeInstanceName(&op->instanceName);
}

static void
freeEnumInstances(XtokEnumInstances * op)
{
  freeArray(&op->propertyList);
}

static void
freeAssociatorNames(XtokAssociatorNames * op)
{
  if (op->objNameSet) {
...
static void
freeReferenceNames(XtokReferenceNames * op)
{
  if (op->objNameSet) {
    freeInstanceName(&op->objectName);
  }
...
static void
freeAssociators(XtokAssociators * op)
{
  if (op->objNameSet) {
    freeInstanceName(&op->objectName);
  }
...
static void
freeReferences(XtokReferences * op)
{
  if (op->objNameSet) {
    freeInstanceName(&op->objectName);
  }
...
static void
freeSetQualifier(XtokSetQualifier * op)
{
...
static void
freeGetProperty(XtokGetProperty * op)
{
  freeInstanceName(&op->instanceName);
}

static void
freeSetProperty(XtokSetProperty * op)
{
  freeInstanceName(&op->instanceName);
...
void
freeCimXmlRequest(RequestHdr hdr)
{
  if (hdr.cimRequest) {
