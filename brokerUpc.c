...
void
lockUpCall(const CMPIBroker * mb)
{
  if (mtx == NULL)
...
void
unlockUpCall(const CMPIBroker * mb)
{
...
/*
 * request_FT is initalized at compile time, so we need to use a macro
 * here to compute the cabability mask 
 */
#ifdef SFCB_INCL_INDICATION_SUPPORT
