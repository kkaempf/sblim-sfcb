...
/*
 * benötigt für die Kommunikation mit den Providern, bzw. der
 * Datenaufbereitung der Ergebnisse eines Providercalls
 */
 ...
 * werden ihre Namen geändert, aus allen vorangestellten "yy" wird
 * "sqfSlq")
 */
...
/*
 * Definition der "Instanzmethoden, damit sie n den Konstruktoren benutzt
 * werden können" 
 */
...
SqlStatement   *
newSqlStatement(int type, ResultSet * rs)
{
...
SubSelect      *
newSubSelectC(SubSelect * sbsA, SubSelect * sbsB, int type)
{
...
Projection     *
newProjection(int mode, UtilList * ul)
{
...
Selection      *
newSelection(UtilList * orderBy, int fetchFirst)
{
...
CrossJoin      *
newCrossJoin(SubSelect * sbsA, SubSelect * sbsB, int type)
{
...
Sigma          *
newSigma(Column * colA, int op, char *value, Column * colB, char *valueB)
{
...
/*
 * Hilfsfunktion, um die Gleichheit zweier Objekte vom Typ Column
 * festzustellen
 */
int
coleql(Column * colA, Column * colB)
{
  // printf("%p %p\n",colA,colB);
  // printf("%p %p\n",colA->colName,colB->colName);
  // printf("%p %p\n",colA->tableAlias,colB->tableAlias);
  if (strcasecmp(colA->colName, colB->colName) == 0
      && strcasecmp(colA->tableAlias, colB->tableAlias) == 0)
    return 0;
  return 1;
}

/*
 * kpiert den Teilbaum, den node repräsentiert in den Baum t ein.
 * rekursiv. 
 */
void
...
/*
 * wird aufgerufen, wenn ein Sigma-Objekt (Bedingung in einer
 * WHERE-Klausel) einem Subselect zugeordnet wird Verteilt die einzelnen
 * Bedinugen auf die eventuell darunterliegenden Subselects (Baumstruktur, 
 * erzeugt durch Joins) 
 */
static void
addWhere(SubSelect * sbs, Sigma * sigma)
{
  ...
  for (col = (Column *) ul->ft->getFirst(ul); col;
       col = (Column *) ul->ft->getNext(ul)) {
...
/*
 * in ul befindet sich eine Liste von Bedingungen, die alle diesem
 * Subselect zugeordnet werden sollen
 */
static void
addWhereListSbs(SubSelect * this, UtilList * ul)
{

  Sigma          *sigma;
  // printf("%p/n",sigma);
  // printf("%d\n",ul->ft->size(ul));
  for (sigma = (Sigma *) ul->ft->getFirst(ul); sigma;
       sigma = (Sigma *) ul->ft->getNext(ul)) {
    // printf("a\n");
    addWhere(this, sigma);
  }
}
...
/*
 * Vergleicht zwei Zeichenketten abhängig von ihrem Typ (lexikographisch, 
 * numerisch, usw.) und der Orderby-Bedingung (asc, desc)
 */
int
row_cmp(char *r1, char *r2, int type, int order)
{
  int             res = 0;
  long long int   a1;
  long long int   b1;
...
/*
 * wird im avlBaum benutzt, wenn Objekte vom Typ Row einsortiert werden.
 * Row entält alle wichtigen Informationen (typ, order), um row_cmp
 * aufrufen zu können
 */
int
item_cmp(const void *item1, const void *item2)
{
  Row            *r1 = (Row *) item1;
  Row            *r2 = (Row *) item2;
  int             i;
  ...
  // dazu sollte mir jemand sagen, wie ich diese Info in getClassDef
  // bekomme
 ...
 * in das Nachrichtenfpormat als Antwort an den Client
 */
static void
addMetaRset(ResultSet * this, Projection * prj)
{
  int             i;
...
/*
 * Destruktoren,...< rufen sich im Prinzip rekursiv auf, so dass ein
 * Aufruf von stmt->free(&stmt) alle im Zusammenhang erzeugten Strukturen
 * wieder freigegeben werden. Noch ausbaufähig
 */
static void
freeRSet(ResultSet ** this)
{
  ResultSet      *t = *this;
  FREE(t->meta);
  FREE(t->query);
...
/*
 * eineee zentrale Funktion. Hier wird der Parsebaum aufebaut: dem
 * statement wir ein value mitgegeben, das durch type eindeutig definiert
 * ist. Abhängig vom Typ wird value auf den Baum angewendet.
 */
static int
addNodeStm(SqlStatement * this, int type, void *value)
{
  ...
  // printf("--- 3\n");
  // falls A u B u C geparst wurde muss ein neues Fullselect oben
  // eingefügt werden
 ...
 * abhängig von der Art des Statements ausgwertet.
 */
static int
calculateStm(SqlStatement * this)
{
  FullSelect     *fs;
  Insert         *ins;
  UpdIns         *tupel;
...
/*
 * nächsten drei Funktionen führen Fullselect-Operaionen auf dem Baum
 * aus.
 */
AvlTree        *
...
/*
 * zwei Select-Statements werden über die Operation verknüpft zu einem.
 * Fallunterschedungen, je nachdem, welchen Typ die einzenen Operanden
 * haben
 */
static AvlTree *
calculateFSel(FullSelect * this, char *db)
{
  ...
  if (this->sigma != NULL) {
    ...
    // printf("NULL2!\n");
    // falls nur select: reorder überflüssig, wird hier dann aber
    // dennoch ausgeführt
    if (this->sigma->orderBy != NULL) {
      // printf("+++++ reorder\n");
...
/*
 * Für die Kodierung im DB Protokoll müssen alle Werte in eine
 * Zeichenkette konvertiert werden
 */
static char    *
value2string(CMPIData d)
{
  char            str[128];
  char           *sp = str;
...
getColumnNr(const char *name, UtilList * ul)
{
  Column         *col;
  int             i;
  for (i = 0, col = (Column *) ul->ft->getFirst(ul); col;
       col = (Column *) ul->ft->getNext(ul), i++) {
...
static int
getColNr(Column * colA, UtilList * ul)
{
  Column         *col;
  int             i;
  for (i = 0, col = (Column *) ul->ft->getFirst(ul); col;
...
/*
 * in eine Row müssen nicht nur die Spalten, die in der Projektion
 * definiert werden gespeichert werden, sondern auch, die, die in einer
 * Bedingung vorkommen select a from X where b<n 
 */
static int
testSigma(Projection * pi, Row * row)
{
  int             res = UNDEF;
...
crossproduct(AvlTree *setA, AvlTree *setB, UtilList * ul, int type)
{
  ...
  // setA->free(&setA);
  // setB->free(&setB);
  // fetch first gibts nicht , da das ja nochmals teil eines joins sein
  // kann, der anders sortiert ist
  return res;
}

/*
 * berechnet eine Ergebnismenge aus allen darunterliegenden Subselects.
 * Rekursiv
 */
static AvlTree *
calculateSSel(SubSelect * this, char *db)
{
  // printf("-- 0\n");
  if (this->isSet != 1) {
    AvlTree        *setA = NULL,
...
{
  // printf("ssffs %p\n",this);
  // auf colA und colB muss kein free aufgerufen werden, da die Cols alle
  // in den Selects behandelt werden 
  Sigma          *t = *this;
  // FREE(t->colA);
  printf("freeSi 2\n");
  // FREE(t->colB);
 ...
 */
CMPIEnumeration *
createEnum(BinRequestContext * binCtx, BinResponseHdr ** resp, int arrLen)
{
  int             i,
                  c,
                  j;
 ...
 * Genauere Erklärung siehe Tabelle in der Arbeit
 */
...
enumInstances(const char *ns, const char *cn)
{
  ...
  if (irc == MSG_X_PROVIDER) {
    ...
    // printf("%d\n",err);
    if (err == 0)
      return createEnum(&binCtx, resp, l);
    return NULL;
  }
  closeProviderContext(&binCtx);
  free(sreq);
...
updateInstance(const char *ns, const char *cn, UtilList * al, UtilList * kl)
{
  ...
  inst = NewCMPIInstance(path, NULL);
  // printf("5\n");
  for (i = 0, el = (ExpressionLight *) al->ft->getFirst(al); el;
...
invokeMethod(const char *ns, const char *cn, const char *mn, UtilList * pl, UtilList * kl)
{
  ...
  for (i = 0, el = (ExpressionLight *) pl->ft->getFirst(pl); el;
...
ClassDef       *
getClassDef(const char *ns, const char *cn)
{
...
processMetaTables(const char *filter, const char *db)
{
  ...
  for (cn = (char *) ul->ft->getFirst(ul); cn;
...
processKeyTable(const char *filter, const char *db)
{
  ...
  for (i = 0, col =
       (Column *) cd->fieldNameList->ft->getFirst(cd->fieldNameList); col;
       col =
       (Column *) cd->fieldNameList->ft->getNext(cd->fieldNameList)) {
    if (col->isKey != KEY)
      continue;
