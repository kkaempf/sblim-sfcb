...
/*
 * $Id: context.c,v 1.13 2009/08/10 22:47:58 mchasal Exp $
 *
...
/*
 * ! This structure stores the information needed to represent contexts
 * for CMPI providers. 
 */
...
// types.
/*
 * ! This structure is used to build linked lists of data containers as
...
__convert2CMPIData(struct native_property *prop, CMPIString **propname)
{
  ...
  if (prop != NULL) {
    ...
    if (propname) {
      *propname = sfcb_native_new_CMPIString(prop->name, NULL, 0);
    }
              ...
              CMPIType type, CMPIValueState state, const CMPIValue * value)
{
  ...
  if (*prop == NULL || (strcmp((*prop)->name, name) == 0)) {
    ...
    if (type == CMPI_chars) {
      ...
      type = CMPI_string;
      v.string = sfcb_native_new_CMPIString((char *) value, NULL, 0);
      value = &v;
    }
