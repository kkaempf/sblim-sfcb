...
static void
__make_NULL(struct native_array *a, int from, int to, int release)
{
  for (; from <= to; from++) {
...
static CMPIArray *
__aft_clone(const CMPIArray *array, CMPIStatus *rc)
{
  CMPIStatus      tmp;
  struct native_array *a = (struct native_array *) array;
...
static CMPICount
__aft_getSize(const CMPIArray *array, CMPIStatus *rc)
{
  struct native_array *a = (struct native_array *) array;
...
static CMPIType
__aft_getSimpleType(const CMPIArray *array, CMPIStatus *rc)
{
  struct native_array *a = (struct native_array *) array;
...
static CMPIData
__aft_getElementAt(const CMPIArray *array, CMPICount index, CMPIStatus *rc)
{
  struct native_array *a = (struct native_array *) array;
...
static CMPIStatus
setElementAt(CMPIArray *array, CMPICount index, const CMPIValue * val,
             CMPIType type, int opt)
{
  ...
  if (index < a->size) {
    ...
    if ((type & (CMPI_ENC | CMPI_ARRAY))
        && (val == NULL || val->chars == NULL)) {
      type = CMPI_null;
    }
    ...
    if (opt || type == a->type) {
      ...
      if (opt) {
        ...
        a->data[index].value = sfcb_native_clone_CMPIValue(type, val, &rc);
      }
      if (localClientMode)
        switch (a->type) {
        case CMPI_instance:
...
static CMPIStatus
__aft_setElementAt(CMPIArray *array, CMPICount index,
...
CMPIStatus
arraySetElementNotTrackedAt(CMPIArray *array,
                            CMPICount index,
...
void
sfcb_native_array_increase_size(const CMPIArray *array,
                                CMPICount increment)
{
  struct native_array *a = (struct native_array *) array;
...
void
native_array_reset_size(const CMPIArray *array, CMPICount increment)
{
  struct native_array *a = (struct native_array *) array;
  a->size = increment;
}
...
static struct native_array *
__new_empty_array(int mm_add,
...
CMPIArray      *
TrackedCMPIArray(CMPICount size, CMPIType type, CMPIStatus *rc)
{
  void           *array =
      internal_new_CMPIArray(MEM_TRACKED, size, type, rc);
  return (CMPIArray *) array;
}

CMPIArray      *
internal_new_CMPIArray(int mode, CMPICount size, CMPIType type,
                       CMPIStatus *rc)
{
...
CMPIArray      *
NewCMPIArray(CMPICount size, CMPIType type, CMPIStatus *rc)
{
  void           *array =
      internal_new_CMPIArray(MEM_NOT_TRACKED, size, type, rc);
  return (CMPIArray *) array;
}

CMPIArray      *
internal_native_make_CMPIArray(CMPIData *av, CMPIStatus *rc,
  ...
  for (i = 0, m = (int) av->value.sint32; i < m; i++)
    if (av[i + 1].state != CMPI_nullValue) {
      if (av[i + 1].type == CMPI_string) {
        char           *chars = (char *) ClObjectGetClString(hdr,
                                                             (ClString *) &
                                                             av[i +
                                                                1].
                                                             value.chars);
        arraySetElementNotTrackedAt((CMPIArray *) array, i,
                                    (CMPIValue *) chars, CMPI_chars);
        ...
        arraySetElementNotTrackedAt((CMPIArray *) array, i, &value,
                                    CMPI_ref);
      } else if (av[i + 1].type == CMPI_instance) {
        value.inst =
            (void *) ClObjectGetClObject(hdr,
                                         (ClString *) & av[i +
                                                           1].value.inst);
        if (value.inst) {
...
CMPIArray      *
native_make_CMPIArray(CMPIData *av, CMPIStatus *rc, ClObjectHdr * hdr)
{
  return internal_native_make_CMPIArray(av, rc, hdr, MEM_TRACKED);
}

CMPIStatus
sfcb_simpleArrayAdd(CMPIArray *array, CMPIValue * val, CMPIType type)
{
  struct native_array *a = (struct native_array *) array;
  if (a->dynamic) {
    if (a->size == 0) {
      a->type = type;
      if (a->type == CMPI_chars)
        a->type = CMPI_string;
    }
