...
main(void)
{
  ...
  for (temp = list1->ft->getFirst(list1); temp;
       temp = list1->ft->getNext(list1))
    printf(" %s \t", temp);
  printf("\n\n");
  printf("Elements in reverse order \n");
  for (temp = list2->ft->getLast(list2); temp;
       temp = list2->ft->getPrevious(list2))
    printf(" %s \t", temp);
  ...
  for (temp = list2->ft->getLast(list2); temp;
    ...
    printf(" %s \t", temp);
  temp = "Element3";
  if (list1->ft->contains(list1, temp))
    printf(" \n\n %s : found in the list \n", temp);
  else
